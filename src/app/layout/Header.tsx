...
const Header = () => {
    ...
    const generateMenu = () => (
        <Menu onClick={clickNav} mode={drawerDisplay ? 'vertical' : 'horizontal'}>
            ...
            <Menu.Item key="NOTE" icon={<FileTextOutlined/>}><a href="https://note.jszoo.com"
                                                                target="_blank">文档</a></Menu.Item>
    ...
    return (
        <div className='common-header'>
            ...
            <Modal
                ...
                footer={null}
            >
                <Search placeholder="关键字模糊搜索..." onSearch={onSearch} allowClear onChange={(e) => {
                ...
                <List
                    ...
                    renderItem={(item) => (
                        <List.Item>
                            <h3 onClick={() => {
                                goDetail(item.id)
                            }} className='searchItem'>{item.title}</h3>
                        </List.Item>
                    )}
                />
            </Modal>
